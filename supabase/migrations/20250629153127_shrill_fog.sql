...
-- Create survey_subscriptions table with time-based plans
CREATE TABLE IF NOT EXISTS survey_subscriptions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid REFERENCES user_profiles(id) ON DELETE CASCADE NOT NULL,
  plan_id text NOT NULL, -- 'daily', 'weekly', 'monthly', 'yearly'
...
BEGIN
  ...
  -- Determine new plan responses based on time-based plans
  CASE new_plan_id
    WHEN 'daily' THEN new_plan_responses := 5;
    WHEN 'weekly' THEN new_plan_responses := 25;
    WHEN 'monthly' THEN new_plan_responses := 100;
    WHEN 'yearly' THEN new_plan_responses := 1000;
    ELSE new_plan_responses := 100; -- Default to monthly plan
  END CASE;
